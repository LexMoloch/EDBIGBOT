...
client.on('messageCreate', async (message) => {
  ...
  if (content.toLowerCase().startsWith('/factionmap')) {
    ...
    await message.reply(`🖼️ Generiram mapu i analizu za **${factionName}** vs **${rivalName}**... može potrajati ~30s`);
    ...
    try {
      ...
      async function fetchAllSystemData(systems) {
        ...
        return allDocs
          .filter(s => s.x !== undefined && s.z !== undefined && s.controlling_minor_faction)
          .map(s => ({
            name: s.name,
            x: s.x, y: s.y, z: s.z,
      ...
      const factions = {
        FACTION: {
          name: factionName,
          colorControlled: "#7CFC00",
          colorUncontrolled: "#228B22",
          prefix: "* "
        },
        RIVAL: {
          name: rivalName,
          colorControlled: "#FF6347",
          colorUncontrolled: "#B22222",
          prefix: "* "
        }
      };

      const canvasWidth = 2000, canvasHeight = 2000, dotRadius = 5;
      const nearbyLimit = 200, labelDistance = 10;
      ...
      const padX = (rawMaxX - rawMinX) * 0.1, padZ = (rawMaxZ - rawMinZ) * 0.1;
      const minX = rawMinX - padX, maxX = rawMaxX + padX;
      const minZ = rawMinZ - padZ, maxZ = rawMaxZ + padZ;
      const scaleX = canvasWidth / (maxX - minX), scaleZ = canvasHeight / (maxZ - minZ);
  ...
  legendItems.forEach((item, i) => {
    ...
    ctx.fillText(item.text, startX + circleRadius * 2 + 8, y);
      ...
      const embed = new EmbedBuilder()
        .setTitle(`${factionName} vs ${rivalName} map`)
        .setColor(0xFFA500)
        .addFields(fields)
      ...
      await message.channel.send({
        embeds: [embed],
        files: [{ attachment: canvas.toBuffer("image/png"), name: "map.png" }]
      });


    } catch (err) {
      console.error(err);
      message.reply(`❌ Generiranje nije uspjelo: ${err.message}`);
    }
  }
